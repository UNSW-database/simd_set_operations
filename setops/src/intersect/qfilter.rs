    ...
    V: SimdBsrVisitor4,
{
    ...
    while i_a < st_a && i_b < st_b {
        ...
        if ms_order != MS_NO_MATCH {
            ...
            let (cmp_mask, and_state) = if ms_order > 0 {
                // Single match
                let match_shuffle = unsafe { *MATCH_SHUFFLE_DICT.get_unchecked(ms_order as usize) };
                ...
                let and_state = (and_masks[0] | and_masks[1]) | (and_masks[2] | and_masks[3]);
    ...
    T: Ord + Copy,
{
    ...
    while i_a < st_a && i_b < st_b {
        ...
        let mut ms_order =
            unsafe { *BYTE_CHECK_MASK_DICT.get_unchecked(bc_mask.to_bitmask() as usize) };
...
fn byte_check(a: i32x4, b: i32x4, prev_mask: mask8x16, index: usize) -> (mask8x16, i32) {
    let (byte_group_a, byte_group_b): (i8x16, i8x16) = unsafe {
        (
            shuffle_epi8(
                convert::<i32x4, i8x16>(a),
                *BYTE_CHECK_GROUP_A_VEC.get_unchecked(index),
            ),
            shuffle_epi8(
                convert::<i32x4, i8x16>(b),
                *BYTE_CHECK_GROUP_B_VEC.get_unchecked(index),
            ),
        )
    };
...
const fn byte_check_mask_to_offset(mask: i32) -> i32 {
    ...
    if offsets[0] == MS_MULTI_MATCH
        || offsets[1] == MS_MULTI_MATCH
        || offsets[2] == MS_MULTI_MATCH
        || offsets[3] == MS_MULTI_MATCH
    {
        MS_MULTI_MATCH
    } else if offsets[0] == MS_NO_MATCH
        && offsets[1] == MS_NO_MATCH
        && offsets[2] == MS_NO_MATCH
        && offsets[3] == MS_NO_MATCH
    {
        MS_NO_MATCH
    } else {
        // Single match
    ...
    if (i_a < st_a) && (i_b < st_b) {
        ...
        loop {
            ...
            if ms_order != -2 {
                let cmp_mask = if ms_order > 0 {
                    let match_shuffle =
                        unsafe { *MATCH_SHUFFLE_DICT.get_unchecked(ms_order as usize) };
                    v_a.simd_eq(shuffle_epi8(v_b, match_shuffle))
                } else {
                    let masks = [
                        v_a.simd_eq(v_b),
