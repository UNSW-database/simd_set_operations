...
use std::{marker::PhantomData, num::Wrapping, ops::BitAnd, simd::cmp::*, simd::*};

use crate::{
    instructions::load_unsafe,
    intersect,
    visitor::{SimdVisitor16, SimdVisitor4, SimdVisitor8, Visitor},
};
    ...
    Simd<S, LANES>: BitAnd<Output = Simd<S, LANES>> + SimdPartialEq<Mask = Mask<S, LANES>>,
{
    ...
    fn fesia_intersect_block<V, I>(&self, other: &Self, base_segment: usize, visitor: &mut V)
    where
        V: SimdVisitor4 + SimdVisitor8 + SimdVisitor16,
        I: SegmentIntersect,
    {
        ...
        while small_offset < self.segment_count() {
            ...
            let and_mask =
                and_result.simd_ne(Mask::<S, LANES>::from_array([false; LANES]).to_int());
            ...
            while mask != 0 {
                ...
                let offset_a =
                    *unsafe { self.offsets.get_unchecked(small_offset + bit_offset) } as usize;
                let offset_b =
                    *unsafe { other.offsets.get_unchecked(large_offset + bit_offset) } as usize;
                let size_a =
                    *unsafe { self.sizes.get_unchecked(small_offset + bit_offset) } as usize;
                let size_b =
                    *unsafe { other.sizes.get_unchecked(large_offset + bit_offset) } as usize;
                ...
                I::intersect(
                    ...
                    unsafe {
                        other
                            .reordered_set
                            .get_unchecked(offset_b..large_reordered_max)
                    },
                    size_a,
                    size_b,
                    visitor,
    ...
    Simd<S, LANES>: BitAnd<Output = Simd<S, LANES>> + SimdPartialEq<Mask = Mask<S, LANES>>,
{
    ...
    fn intersect_k<F: AsRef<Self>>(sets: &[F], visitor: &mut impl Visitor<i32>) {
        debug_assert!(sets
            .windows(2)
            .all(|s| s[1].as_ref().segment_count() >= s[0].as_ref().segment_count()));
        debug_assert!(sets
            .windows(2)
            .all(|s| s[1].as_ref().segment_count() % s[0].as_ref().segment_count() == 0));
            ...
            while mask != 0 {
                ...
                merge_k(
                    sets.iter().map(|set| {
                        ...
                        let offset =
                            unsafe { *set.offsets.get_unchecked(segment_index + bit_offset) }
                                as usize;
                        let size = unsafe { *set.sizes.get_unchecked(segment_index + bit_offset) }
                            as usize;
...
pub trait SegmentIntersect {
    fn intersect<V>(set_a: &[i32], set_b: &[i32], size_a: usize, size_b: usize, visitor: &mut V)
    where
        V: SimdVisitor4 + SimdVisitor8 + SimdVisitor16;
}
    ...
    I: Iterator<Item = &'a [T]>,
{
    ...
    'target_loop: for &target in set_spans[0] {
        'set_loop: for set in &mut set_spans[1..] {
