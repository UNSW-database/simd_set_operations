    ...
    V: Visitor<T>,
{
    ...
    while left.len() >= S && right.len() >= S {
        if left[0] == right[0] || left[0] == right[1] || left[0] == right[2] || left[0] == right[3]
        {
            visitor.visit(left[0]);
        }
        if left[1] == right[0] || left[1] == right[1] || left[1] == right[2] || left[1] == right[3]
        {
            visitor.visit(left[1]);
        }
        if left[2] == right[0] || left[2] == right[1] || left[2] == right[2] || left[2] == right[3]
        {
            visitor.visit(left[2]);
        }
        if left[3] == right[0] || left[3] == right[1] || left[3] == right[2] || left[3] == right[3]
        {
    ...
    T: Ord + Copy,
{
    ...
    while i_a < st_a && i_b < st_b {
        ...
        let byte_check_mask0 = simd_swizzle!(convert::<i32x4, i8x16>(v_a), BYTE_CHECK_GROUP_A[0])
        ...
        if (wc_mask0 & 0b0011) != 0 {
            visitor.visit(unsafe { *set_a.get_unchecked(i_a + 0) })
        }
        if (wc_mask0 & 0b1100) != 0 {
            visitor.visit(unsafe { *set_a.get_unchecked(i_a + 1) })
        }

        let wc_mask1: u64 = word_check_mask1.to_bitmask();
        if (wc_mask1 & 0b0011) != 0 {
            visitor.visit(unsafe { *set_a.get_unchecked(i_a + 2) })
        }
        if (wc_mask1 & 0b1100) != 0 {
            visitor.visit(unsafe { *set_a.get_unchecked(i_a + 3) })
        }
    ...
    T: Ord + Copy,
{
    ...
    while i_a < st_a && i_b < st_b {
        ...
        let byte_group_a = shuffle_epi8(v_a0, BMISS_STTNI_BC_ARRAY[0])
            | shuffle_epi8(v_a1, BMISS_STTNI_BC_ARRAY[1]);
        let byte_group_b = shuffle_epi8(v_b0, BMISS_STTNI_BC_ARRAY[0])
            | shuffle_epi8(v_b1, BMISS_STTNI_BC_ARRAY[1]);

        let bc_mask: i32x4 = unsafe {
            _mm_cmpestrm(
                byte_group_b.into(),
                8,
                byte_group_a.into(),
