    ...
    T: Ord + Copy,
{
    result.iter().all(|result_item| {
        sets.iter()
            .all(|input_set| input_set.as_ref().contains(&result_item))
    })
}

// If an item is common, then it is in the result.
pub fn prop_all_common_items_in_result<S, T>(result: &Vec<T>, sets: &[S]) -> bool
where
    S: AsRef<[T]>,
    T: Ord + Copy,
{
    for item in sets[0].as_ref() {
        if sets.iter().skip(1).all(|set| set.as_ref().contains(item)) {
