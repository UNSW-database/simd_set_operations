        ...
        for algorithm in algorithms:
            try:
                ...
                def element_count(row):
                    ...
                    sumVar = sum([elements_in(set_idx, skew, max_len) for set_idx in range(set_count)])
                    return sumVar
                val = df.apply(element_count, axis=1)
                df["element_count"] = val
                ...
                if "branch_miss_rate" in df and "branch_misses" in df and "branches" in df:
                    df["branch_miss_rate"] = df["branch_misses"] / df["branches"]
                if "ipc" in df and "instructions" in df and "cpu_cycles" in df:
                    df["ipc"] = df["instructions"] / df["cpu_cycles"]
                if "cpi" in df and "cpu_cycles" in df and "instructions" in df:
