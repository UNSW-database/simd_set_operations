...
use benchmark::{datafile, fmt_open_err, path_str, schema::*};
use clap::Parser;
use colored::Colorize;
use setops::intersect::{self, run_svs};
...
fn dataset_info(cli: &Cli, dataset_name: &str) -> Result<DatasetInfo, String> {
    ...
    let info_file = fs::File::open(&info_path).map_err(|e| fmt_open_err(e, &info_path))?;

    let dataset_info: DatasetInfo = serde_json::from_reader(info_file).map_err(|e| {
        format!(
            "invalid json file {}: {}",
            path_str(&info_path),
            e.to_string()
...
fn verify_dataset(info: &DatasetInfo, dir: &PathBuf) -> Result<(), String> {
    ...
    for x in benchmark::xvalues(info) {
        ...
        let pairs = fs::read_dir(&xdir).map_err(|e| fmt_open_err(e, &xdir))?;

        for (i, pair_path) in pairs.enumerate() {
            let pair_path = pair_path.map_err(|e| {
                format!(
            ...
            let datafile =
                File::open(&datafile_path).map_err(|e| fmt_open_err(e, &datafile_path))?;
            ...
            match &info.dataset_type {
                DatasetType::Synthetic(s) => verify_synthetic(&sets, &benchmark::props_at_x(s, x)),
                DatasetType::Real(_) => verify_real(&sets, x),
            }
        }
        println!();
    }
...
fn verify_sizes(sets: &[Vec<i32>], info: &IntersectionInfo) {
    ...
    let lengths_ascending = sets.windows(2).all(|s| s[0].len() <= s[1].len());
...
fn verify_density(sets: &[Vec<i32>], info: &IntersectionInfo) {
    ...
    let actual_max = *sets.iter().map(|s| s.iter().max().unwrap()).max().unwrap();
...
fn verify_selectivity(sets: &[Vec<i32>], selectivity: u32) {
    let smallest_len = sets.iter().map(|s| s.len()).min().unwrap();

    assert!(sets[0].len() == smallest_len);

    let result_len = run_svs(&sets, intersect::branchless_merge).len();
...
fn trace_density(sets: &[Vec<i32>]) {
    let max_len = sets.iter().map(|s| s.len()).max().unwrap();

    let max_value = *sets.iter().map(|s| s.iter().max().unwrap()).max().unwrap();
