...
fn try_parse_twoset_c(name: &str) -> Option<Timer> {
    ...
    maybe_intersect.map(|i| Timer {
        twoset: Some(Box::new(move |warmup, a, b| {
            Ok(harness::time_twoset_c(warmup, a, b, i))
        })),
        kset: Some(Box::new(move |warmup, sets| {
            harness::time_svs_c(warmup, sets, i)
        })),
    })
}
...
impl TwosetTimingSpec<UnsafeWriter<i32>> for UnsafeWriter<i32> {
    fn twoset_timer(i: Intersect2<[i32], UnsafeWriter<i32>>) -> Timer {
        Timer {
            ...
            kset: Some(Box::new(move |warmup, sets| {
                harness::time_svs::<UnsafeWriter<i32>>(warmup, sets, i)
            })),
        }
    }
}
...
fn try_parse_bsr(name: &str) -> Option<Timer> {
    ...
    maybe_intersect.map(|intersect: UnsafeIntersectBsr| Timer {
        twoset: Some(Box::new(move |warmup, a, b| {
            Ok(harness::time_bsr(warmup, a, b, intersect))
        })),
        kset: None,
    })
}
    ...
    V: SimdVisitor4 + SimdVisitor8 + SimdVisitor16 + 'static,
{
    ...
    maybe_intersect.map(|intersect| Timer {
        twoset: None,
        kset: Some(Box::new(move |warmup, sets| {
            harness::time_kset(warmup, sets, intersect)
        })),
    })
}
...
fn try_parse_roaring(name: &str, count_only: bool) -> Option<Timer> {
    match name {
        "croaring_opt" => Some(Timer {
            twoset: Some(Box::new(move |warmup, a, b| {
                Ok(harness::time_croaring_2set(warmup, a, b, count_only, true))
            })),
            kset: if count_only {
                None
            } else {
                Some(Box::new(|warmup, sets| {
                    Ok(harness::time_croaring_svs(warmup, sets, true))
                }))
            },
        }),
        "croaring" => Some(Timer {
            twoset: Some(Box::new(move |warmup, a, b| {
                Ok(harness::time_croaring_2set(warmup, a, b, count_only, false))
            })),
            kset: if count_only {
                None
            } else {
                Some(Box::new(|warmup, sets| {
                    Ok(harness::time_croaring_svs(warmup, sets, false))
                }))
            },
        }),
    ...
    V: Visitor<i32> + SimdVisitor4 + SimdVisitor8 + SimdVisitor16 + HarnessVisitor,
{
    ...
    use FesiaTwoSetMethod::*;
    let (intersect, rest) = if prefix.starts_with(FESIA_HASH) {
    ...
    use SimdType::*;
    let maybe_timer: Option<Timer> = match rest {
        "8" => Some(gen_fesia_timer::<MixHash, i8, 16, V>(
            hash_scale, intersect, Sse,
        )),
        "16" => Some(gen_fesia_timer::<MixHash, i16, 8, V>(
            hash_scale, intersect, Sse,
        )),
        "32" => Some(gen_fesia_timer::<MixHash, i32, 4, V>(
            hash_scale, intersect, Sse,
        )),
        _ => None,
    };
