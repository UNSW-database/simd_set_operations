...
pub fn load_sets(root: &PathBuf, source: &str) -> Result<Vec<Vec<i32>>, String> {
    ...
    let sets = if let Ok(cache) = File::open(&cache_path) {
        println!("Using cache");
        datafile::from_reader(cache).map_err(|e| {
            format!(
                "unable to parse {}: {}",
                path_str(&cache_path),
                e.to_string()
...
fn parse_and_cache_webdocs(
    ...
    let text_file = File::open(&text_path)
        .map_err(|e| fmt_open_err(e, &text_path) + ", did you run ./scripts/fetch_*.bash ?")?;
    ...
    let cache = File::create(cache_path)
        .map_err(|e| format!("unable to write datafile: {}", e.to_string()))?;

    datafile::to_writer(cache, &sets)
        .map_err(|e| format!("unable to parse datafile: {}", e.to_string()))?;
...
fn parse_text(text: File) -> Result<Vec<DatafileSet>, String> {
    ...
    reader
        .lines()
        .map(|line| {
            parse_line(line.map_err(|e| format!("unable to read line: {}", e.to_string()))?)
        })
        .collect()
}
...
fn generate_real_intersection(
    ...
    let mut sets: Vec<&DatafileSet> = all_sets.choose_multiple(rng, set_count).collect();
